...
test("validateCsvAttachment extracts addresses with 1-based row numbers", () => {
  ...
  assert.deepEqual(result.addresses[0], { rowNumber: 2, address: "1 Test Street SYDNEY NSW 2000", ownerName: null, ownerEmail: null, currentRent: null });
  assert.deepEqual(result.addresses[1], { rowNumber: 3, address: "2 Sample Road PARRAMATTA NSW 2150", ownerName: null, ownerEmail: null, currentRent: null });
});
...
test("validateCsvAttachment carries an optional owner email for each address row", () => {
  const result = validateCsvAttachment({
    fileName: "batch.csv",
    content: "address,Owner Name,Email\n1 Test Street SYDNEY NSW 2000,Alice Example,Alice@Example.com\n",
  });
  assert.equal(result.addresses[0]?.ownerEmail, "alice@example.com");
});

test("validateCsvAttachment reads a current weekly rent from a formatted CSV value", () => {
  const result = validateCsvAttachment({
    fileName: "batch.csv",
    content: "address,current rent\n1 Test Street SYDNEY NSW 2000,$850/week\n",
  });
  assert.equal(result.addresses[0]?.currentRent, 850);
});
